...
void VulkanEngine::init(const bool _ROMFolderWasFound)
{
	ROMFolderWasFound = _ROMFolderWasFound;

	Wireframe::Window::DesktopWindow_CreateInfo windowInfo;
	windowInfo.size = { 1700 , 900 }; windowInfo.title = "Projekt Swerve";
...
void VulkanEngine::draw()
{
	...
	if (SDL_GetWindowFlags(window._window) & SDL_WINDOW_MINIMIZED)
		return;
	...
	VK_CHECK(vkAcquireNextImageKHR(GPU.device, _swapchain, 1000000000, _presentSemaphore, nullptr, &swapchainImageIndex));
	...
	VkRenderPassBeginInfo rpInfo = vkinit::renderpass_begin_info(_renderPass, window._windowExtent, _framebuffers[swapchainImageIndex]);
...
void VulkanEngine::run()
{
	...
	while (!bQuit)
	{
		...
		while (SDL_PollEvent(&e) != 0)
		{
			...
			ImGui_ImplSDL2_ProcessEvent(&e); //ImGUI input
		}
...
void VulkanEngine::init_framebuffers()
{
	...
	VkFramebufferCreateInfo fb_info = vkinit::framebuffer_create_info(_renderPass, window._windowExtent);
	...
	for (int i = 0; i < swapchain_imagecount; i++) {
		...
		VK_CHECK(vkCreateFramebuffer(GPU.device, &fb_info, nullptr, &_framebuffers[i]));

		_mainDeletionQueue.push_function([=]() {
			vkDestroyFramebuffer(GPU.device, _framebuffers[i], nullptr);
...
void VulkanEngine::init_commands()
{
	...
	VK_CHECK(vkCreateCommandPool(GPU.device, &commandPoolInfo, nullptr, &_commandPool));

	//allocate the default command buffer that we will use for rendering
	VkCommandBufferAllocateInfo cmdAllocInfo = vkinit::command_buffer_allocate_info(_commandPool, 1);

	VK_CHECK(vkAllocateCommandBuffers(GPU.device, &cmdAllocInfo, &_mainCommandBuffer));

	_mainDeletionQueue.push_function([=]() {
		vkDestroyCommandPool(GPU.device, _commandPool, nullptr);
	});
}
...
void VulkanEngine::init_sync_structures()
{
	...
	VK_CHECK(vkCreateFence(GPU.device, &fenceCreateInfo, nullptr, &_renderFence));

	//enqueue the destruction of the fence
	_mainDeletionQueue.push_function([=]() {
		vkDestroyFence(GPU.device, _renderFence, nullptr);
		});
	VkSemaphoreCreateInfo semaphoreCreateInfo = vkinit::semaphore_create_info();

	VK_CHECK(vkCreateSemaphore(GPU.device, &semaphoreCreateInfo, nullptr, &_presentSemaphore));
	VK_CHECK(vkCreateSemaphore(GPU.device, &semaphoreCreateInfo, nullptr, &_renderSemaphore));
	
	//enqueue the destruction of semaphores
	_mainDeletionQueue.push_function([=]() {
		vkDestroySemaphore(GPU.device, _presentSemaphore, nullptr);
		vkDestroySemaphore(GPU.device, _renderSemaphore, nullptr);
		});
}
...
void VulkanEngine::init_pipelines()
{
	...
	VK_CHECK(vkCreatePipelineLayout(GPU.device, &pipeline_layout_info, nullptr, &_trianglePipelineLayout));
	...
	pipelineBuilder._viewport.width = (float)window._windowExtent.width;
	pipelineBuilder._viewport.height = (float)window._windowExtent.height;
	pipelineBuilder._viewport.minDepth = 0.0f;
	pipelineBuilder._viewport.maxDepth = 1.0f;

	pipelineBuilder._scissor.offset = { 0, 0 };
	pipelineBuilder._scissor.extent = window._windowExtent;
	...
	_trianglePipeline = pipelineBuilder.build_pipeline(GPU.device, _renderPass);
	...
	_redTrianglePipeline = pipelineBuilder.build_pipeline(GPU.device, _renderPass);
	...
	VK_CHECK(vkCreatePipelineLayout(GPU.device, &mesh_pipeline_layout_info, nullptr, &_meshPipelineLayout));

	//hook the push constants layout
	pipelineBuilder._pipelineLayout = _meshPipelineLayout;
	//build the mesh triangle pipeline
	_meshPipeline = pipelineBuilder.build_pipeline(GPU.device, _renderPass);

	vkDestroyShaderModule(GPU.device, meshVertShader, nullptr);
	vkDestroyShaderModule(GPU.device, redTriangleVertShader, nullptr);
	vkDestroyShaderModule(GPU.device, redTriangleFragShader, nullptr);
	vkDestroyShaderModule(GPU.device, triangleFragShader, nullptr);
	vkDestroyShaderModule(GPU.device, triangleVertexShader, nullptr);

	_mainDeletionQueue.push_function([=]() {
		vkDestroyPipeline(GPU.device, _redTrianglePipeline, nullptr);
		vkDestroyPipeline(GPU.device, _trianglePipeline, nullptr);
		vkDestroyPipeline(GPU.device, _meshPipeline, nullptr);

		vkDestroyPipelineLayout(GPU.device, _trianglePipelineLayout, nullptr);
		vkDestroyPipelineLayout(GPU.device, _meshPipelineLayout, nullptr);
	});
}
